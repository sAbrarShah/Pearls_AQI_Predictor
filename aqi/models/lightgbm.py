...
def train_and_register() -> dict[str, Any]:
    ...
    with mlflow.start_run():
        ...
        for h in range(DAYS_AHEAD):
            ...
            models.append(m)
            bi = int(getattr(m, "best_iteration_", 0) or 0)
        ...
        local_path = os.path.join(ARTIFACTS_DIR, "latest_lightgbm.joblib")
...
def forecast_3days() -> dict[str, Any]:
    p = os.path.join(ARTIFACTS_DIR, "latest_lightgbm.joblib")
    if not os.path.exists(p):
        raise RuntimeError(f"Missing LightGBM artifact: {p}")
